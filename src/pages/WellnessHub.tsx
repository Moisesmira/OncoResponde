...
const topics = [
  ...
  { id: 'relajate', icon: '🎧', title: 'Escuchar y relajarte', text: 'Audios guiados, micro-podcasts y sonidos suaves para acompañarte.' },
];
      ...
      <main className="wellness-page" id="main-content">
        ...
        <section className="wellness-grid" aria-label="Áreas de bienestar">
          {topics.map((topic) => (
            <Link
              key={topic.id}
              to={topic.id === 'respiracion' ? '/respiracion' : topic.id === 'relajate' ? '/escuchar' : `/cuidate/${topic.id}`}
